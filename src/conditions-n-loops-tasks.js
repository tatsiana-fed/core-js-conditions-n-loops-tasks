...
function convertToRomanNumerals(num) {
  ...
  if (num <= 30) {
    return `XX${roman[num % 10]}`;
  }
  if (num <= 40) {
    return `XXX${roman[num % 10]}`;
  }
  return '';
}
...
function shuffleChar(str, iterations) {
  ...
  while (i <= iterations) {
    ...
    for (let j = 0; j < str.length; j += 1) {
      if (j % 2 === 0) nonOdd += newString[j];
      if (j % 2 === 1) odd += newString[j];
    }
    newString = nonOdd + odd;
    if (newString === str) return shuffleChar(str, iterations % i);
    i += 1;
  }
  return newString;
}
...
function getNearestBigger(number) {
  const array = Array.from(`${number}`, (el) => Number(el));
  let index;
  let element;
  ...
  const leftArray = array.splice(0, index);
  const rightPart = array.sort((a, b) => a - b);
  let nextElement;
  let indexNextElement;
  i = 0;
  while (i < rightPart.length) {
    if (rightPart[i] === element) {
      nextElement = rightPart[i + 1];
      indexNextElement = i + 1;
    }
    i += 1;
  }
  const rightArray = [
    ...rightPart.splice(0, indexNextElement),
    ...rightPart.splice(1),
  ];
  const result = +[...leftArray, nextElement, ...rightArray].join('');
  return result;
}
